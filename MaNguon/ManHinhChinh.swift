...
final class ManHinhChinh: UIViewController, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    private func thongKe(){let homNay=Date(),lich=TienIchNgay.lich;var cong=0.0,phanTram=0.0,ngayChuanThang=0,ngayCon=0
        ...
        tongKet.text=String(format:"  THỐNG KÊ THÁNG\n  Giờ: %.2f / %.0f  •  Công: %.2f / %.2f\n  Thiếu: %.2f công  •  Sản lượng: %.2f%% / %.2f%%\n  Còn thiếu: %.2f%%  •  Cần/ngày: %.2f%%",cong*8,congChuan*8,cong,congChuan,thieu,phanTram,muc,conThieu,moiNgay)
    }
}
